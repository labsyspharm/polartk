...
if __name__ == '__main__':
    ...
    output_filepath = os.path.expanduser('~/polar_data/mean_intensity.csv')
    ...
    for data_filepath in tqdm.tqdm(glob.glob(data_filepattern)):
        ...
        df = df.groupby(by=['cellid', 'label'])['intensity'].mean()
        channelid = os.path.splitext(os.path.basename(data_filepath))[0].split('_')[1]
        df = df.reset_index(name='mean_intensity_ch{}'.format(channelid))
